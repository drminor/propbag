...
namespace DRM.WrapperGenLib
{
    ...
    public static class WrapperGenerator<T> where T : IWrapperBase
    {
        public static Type EmitWrapper(TypeDescription typeDescription, IModuleBuilderInfo moduleBuilderInfo)
        {
            ...
            var fieldBuilders = new Dictionary<string, PropertyEmitter<T>>();
            foreach(var property in propertiesToImplement)
            {
                if(fieldBuilders.TryGetValue(property.Name, out PropertyEmitter<T> propertyEmitter))
                {
                ...
                {
                    propertyEmitter = new PropertyEmitter<T>(typeBuilder, property); //, propertyChangedField));
